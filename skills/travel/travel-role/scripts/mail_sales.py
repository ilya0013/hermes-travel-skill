...
Владелец подписался на рассылки перевозчиков (17–18.09.2026), фильтр Gmail сам настроить не смог, а
фильтры через Gmail API требуют права `gmail.settings.basic`, которого в токене агента нет — потому
уборку делает код правом `gmail.modify`: письма с доменов `SENDERS` за `--days` дней, ещё без ярлыка,
получают ярлык и теряют `INBOX` одним `batchModify`. Ярлык ищется по имени, нет — создаётся.
Cron Hermes зовёт каждый час без агента (обёртка `/opt/data/scripts/travel/mail_sales.sh`, доставка
`local`): stdout — одна строка «сколько и от кого», пустой — нечего убирать; ошибка — код 1 и stderr.
Разбор писем (цена, «продажа до», «полёт когда» → Sheet «Распродажи») — следующий кусок, когда акции
