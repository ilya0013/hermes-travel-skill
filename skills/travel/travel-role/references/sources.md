...
| `report.py --run R --title … --variant "A=1+3" [--failed id=причина] [--no-drive]` (питон Hermes, `HERMES_HOME=/opt/data`) | `nbp_api`, `CALC` | курс раз на прогон, злотые, дорога до двери по `profile.yaml`, ★ дешевейшему до двери, «дешевле в журнале», блок источников, таблица всех строк в Google Doc папки «Поездки»; `--list` — строки с номерами | — | ничего не ищет: только считает и показывает строки журнала |
