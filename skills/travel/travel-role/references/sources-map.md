...
- **Wizz Air** → DataDome-капча: сначала «Let's confirm you are human / Begin», затем картинки («Choose all the clocks»). Капчу не обходить; цена плеча Wizz на дату — `google_flights.py` (равна сайту, проверено 14.09.2026). Форма покупки по шаблонной ссылке `wizzair.com/en-gb/booking/select-flight/<откуда>/<куда>/<дата>/null/1/0/0/null` у человека открывается с подставленными рейсом и датой (проверено 14.09.2026); из браузера агента та же форма открывается, но список рейсов не грузится (429) — ссылку отдавать, не открывать.
...
- Снято агентом 11.09.2026 (прогоны Барселона/Вена, перенесено из его блокнота 14.09): **lot.com** —
...
- **Google Flights в браузере, запрос словами `?q=…for 1 adult`** (снято агентом 11.09.2026,
...
- **Ryanair select-страница one-way** (снято агентом 14.09.2026, WMI→BCN): один клик «Wybierz»
